...
/**
 * A Preste source file cannot be read in one go and then separately parsed.
 * Instead, each expression in the root of the source file must be read and
 * evaluated before reading the next expression. In other words, out evaluation
 * semantics for a source file are essentially aligned with the interactive
 * REPL.
 * <p>
 * 
 */
public final class PresteReplNode extends RootNode {

  private final Map<String, RootCallTarget> functions;
  @CompilationFinal
  private boolean registered;

  private final ContextReference<PresteContext> reference;
