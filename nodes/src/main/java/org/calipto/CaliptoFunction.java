...
/**
 * Represents a Calipto function. In the language a function is just a value of a
 ...
 * help of a Truffle {@link Assumption}: a call node can keep the call target
 * returned by {@link #getCallTarget()} cached until the assumption returned by
 * {@link #getCallTargetStable()} is valid.
 * <p>
 * The {@link #callTarget} can be {@code null}. To ensure that only one
 * {@link SLFunction} instance per name exists, the {@link SLFunctionRegistry}
 * creates an instance also when performing name lookup. A function that has
 * been looked up, i.e., used, but not defined, has a call target that
 * encapsulates a {@link SLUndefinedFunctionRootNode}.
 */
@ExportLibrary(InteropLibrary.class)
public final class CaliptoFunction implements TruffleObject {
  public static final int INLINE_CACHE_SIZE = 2;

  private static final TruffleLogger LOG = TruffleLogger
      .getLogger(CaliptoLanguage.ID, CaliptoFunction.class);
  ...
  protected CaliptoFunction(CaliptoLanguage language, String namespace, String name) {
  ...
  /**
   * {@link CaliptoFunction} instances are always visible as executable to other
   * languages.
   */
