...
public class CaliptoContext {
  ...
  public CaliptoContext(
    ...
    this.topScopes = singleton(
        Scope
            .newBuilder("global", functionRegistry.getFunctionsObject())
            .build());

    concat(getIntrinsics(), externalIntrinsics.stream())
        .forEach(this::installIntrinsic);
    concat(getSideEffects(), externalSideEffects.stream())
        .forEach(this::installSideEffect);
  }
  ...
  public void installIntrinsic(NodeFactory<? extends IntrinsicNode> factory) {
    ...
    getFunctionRegistry()
        .register(name, Truffle.getRuntime().createCallTarget(rootNode));
  }
  ...
  public Object fromForeignValue(Object a) {
    if (a instanceof Long || a instanceof String || a instanceof Boolean) {
      return a;
    } else if (language.getSymbols().isSymbol(a)) {
  ...
  private Object fromForeignSymbol(Object a) {
    return language.getSymbols().internSymbol(a);
  }
